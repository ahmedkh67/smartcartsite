export const Navlist = [
    {
        title: " الرئيسية ",
        link: "././"
    },
    {
        title: " خدماتنا ",
        link: "/Servicespage"
    },
    {
        title: " تواصل معنا ",
        link: "/Contacts"
    },
]

export const SlidesImages = {
  HeroSlides : {
    Slide1: "slide11", 
    Slide2: "slide22",
    Slide3: "slide33",
    Slide4: "slide44", 
    Slide5: "slide55",
    Slide6: "slide66",
    Slide7: "slide77", 
    Slide8: "slide88",
    Slide9: "slide99",
  },

  DecorativetoolsSlides : {
    Slide1: "image9", 
    Slide2: "image4",
    Slide3: "image5",
    Slide4: "image9", 
    Slide5: "image4",
    Slide6: "image5",
    Slide7: "image9", 
    Slide8: "image4",
    Slide9: "image5",
  },
  LightsequipmentSlides : {
    Slide2: "image6",
    Slide1: "image7", 
    Slide3: "slide77",
    Slide4: "image6",
    Slide5: "image7", 
    Slide6: "slide77",
    Slide7: "image6",
    Slide8: "image7", 
    Slide9: "slide77",
  },
  LuxurywaresSlides : {
    Slide1: "mainback4", 
    Slide2: "image11",
    Slide3: "image12",
    Slide4: "mainback4", 
    Slide5: "image11",
    Slide6: "image12",
    Slide7: "mainback4", 
    Slide8: "image11",
    Slide9: "image12",
  },
  SpecialeqipmentSlides : {
    Slide1: "mainback3", 
    Slide2: "image8",
    Slide3: "image5",
    Slide4: "mainback3", 
    Slide5: "image8",
    Slide6: "image5",
    Slide7: "mainback3", 
    Slide8: "image8",
    Slide9: "image5",
  },
  DeliverysrvcSlides : {
    Slide1: "image15", 
    Slide2: "image14",
    Slide3: "image13",
    Slide4: "image15", 
    Slide5: "image14",
    Slide6: "image13",
    Slide7: "image15", 
    Slide8: "image14",
    Slide9: "image13",
  },
  
}


export const Moreboxes = [
    {
        MoreBoxtitle: " الريادة في سوق الترفيه ",
        MoreBoxbio: " تعزيز مكاننا كشركة رائدة في قطاع الترفيه من خلال تقديم خدمات و منتجات مبتكرة ",
        MoreBoxsrc: '/QualityIcon.svg'
    },
    {
      MoreBoxtitle: " التطوير المستمر ",
      MoreBoxbio: " الاستثمار في احدث التقنيات و تطوير منتاجتنا بما يتناسب مع احتياجات السوق ",
      MoreBoxsrc: "/TimeIcon.svg"
    },
    {
        MoreBoxtitle: " رضا العملاء ",
        MoreBoxbio: " ضمان تجربة استثنائية تفوق توقعات العملاء من خلال خدماتنا ذات الجودة العالية ",
        MoreBoxsrc: "/TestIcon.svg"
    },
    
    {
        MoreBoxtitle: " التوسع الجغرافي ",
        MoreBoxbio: " فتح فروع جديدة وتوسيع شبكة اعمالنا في مختلف مناظق المملكة ",
        MoreBoxsrc: "/location.svg"
    },
    
]

export const ServicesCards = [
  { title: " قطارات الاطفال الترفيهية ",
    bio: " قطارات مصممة خصيصا للأطفال داخل المولات والمراكز التجارية , تتميز ب تصاميم منة و عصرية تضمن تجربة ممتعة و مثيرة ",
    link: "/Decorativetools ",
    src: "mainback1"
    
  },
  
  { title: " عربات الدفع اليدوي" ,
    bio: " عربات دفع مريحة و عملية مصممة للاهالي لنقل اطفالهم بسهولة و امان داخل المولات",
    link: "/Specialequipment  ",
    src: "mainback3"
    
  },
  { title: " العربات الكهربائية ",
    bio: " عربات كهربائية امنة تحاكي تصاميم السيارات الوافعية تتميز بوسائل الامان و التحكم عن بعد",
    link: "/Luxurywares  ",
    src: "mainback4"
    
  },
  { title: " ادارة و تشغيل المراكز الترفيهية ",
    bio: " نقدم خدمات متكاملة في تصميم و ادارة و تشغيل المراكز الترفيهية لضمان تجربة مميزة و مثيرة للزوار ",
    link: "/Deliverysrvc",
    src: "mainback4"
    
  },
] 

export const HeroSldieText = {
  title: "     العربة الذكية للترفيه   ", 
  bio: " متخصصون في تقديم حلول ترفيهية مبتكرة تجمع بين الأمان والمتعة للاطفال ولعائلات , نعمل بجهد لنكون الشريك الأول للمولات و المراكز التجارية في المملكة العربية السعودية "
}


export const DecorativetoolsSlideText = {
title : "           تأجير أدوات الزينة و الديكور " , 
bio: "        نحن متخصصون في تأجير لوازم الحفلات والمناسبات بجودة عالية وبأسعار تنافسية. سواء كنت تخطط لحفل زفاف، أو عيد ميلاد، أو مناسبة خاصة، فإننا نقدم لك كل ما تحتاجه لجعل يومك مميزاً ومختلفاً", 
}

export const LightsequipmentSlideText = {
title : "                       تأجير أدوات الصوت و الاضاءة ",
bio : "            سواء كان حفل او محاضرة , نوفر لك احدث انظمة الصوت و الاضاءة التي تضمن لك تجربة مميز ",
}

export const LuxurywaresSlidesText = {
  title : "                     تأجير الأواني و القطع الفاخرة ",
  bio : "            تأثيث الطاولات بأواني مميزة و أسطوانات تقديم أنيقة , لكي تضفي لمسة فاخرة على مائدتك "
}

export const SpecialequipmentSlideText = {
title : " تأجير الأثاث و المعدات الخاصة",
bio : " من الأرائك الفاخرة الى المفروشات المريحة , نقدم لك خبارات متنوعة تتناسب مع ذوقك و احتياجك ",
}

export const DeliverysrvcSlidesText = {
  title : " خدمة التوصيل و التركيب ", 
  bio : " نحن نهتم بكل التفاصيل , من التوصيل الى التركيب , لضمان ان كل شيء جاهز قبل موعد الحدث " ,
}



export const NumBoxes = [
  {
      Num: "+300" ,
      Dis: " عميل ",
      Src: "/Person.svg"
  },
  {
      Num: "+200" ,
      Dis: " منتج",
      Src: "/cart.svg"
  },
  {
      Num: "+7 " ,
      Dis: " أعوام ",
      Src: "/Years.svg"
  },
]




export const SocialBoxes = [
  {
      title: " انستغرام " ,
      Src: "/insta.svg",
      link: "https://www.instagram.com/smartcart2020?igsh=bnZ3NnF2MTF0eTcx"
  },
  {
      title: " البريد " ,
      Src: "/gmail.svg",
      link: "mailto:ALHARBAI_10@hotmail.com"
  },
  {
      title: " واتساب " ,
      Src: "/Whatsapp Icon.svg",
      link: "https://wa.me/+966543367208",
  },
]

export const Commentsecc = [
  {
      name: "ahmed mohamed",
      rate: 5 ,
      comment: " الموظفين محترفين في التعامل و الانجاز و طريقتهم جدا جميلة",
      src: "/img2.jpeg" 
      
  },
  
  {
      name: "abdullah Jmel",
      rate: 4 ,
      comment: " اول مرة اتعامل معهم و ما ندمت ابدا صراحة ",
      src: "/img2.jpeg" 
      
  },
  {
    name: "khaled 88k",
    rate: 4 ,
    comment: " سريعين في الانجاز و اسعارهم جدا مناسبة  ",
    src: "/img3.jpeg" 
    
  },
  {
    name: "khaled 88k",
    rate: 4 ,
    comment: " سريعين في الانجاز و اسعارهم جدا مناسبة  ",
    src: "/img3.jpeg" 
    
  },
]



export const Maps = {   
  
  }




export const footerLinks = [
    {
      title: " الصفحات ",
      links: [
        { title: " الرئيسية ", url: "./" },
        { title: " أعمالنا ", url: "/Projectspage" },
        { title: " تواصل معنا ", url: "/Contacts" },
       
      ],
    },
    {
      title: " روابط سريعة ",
      links: [
        { title: " ابرز الأعمال ", url: "/Projectspage" },       
        { title: " الاستفسارات ", url: "https://wa.me/+966590029919" },
      ],
    },
    {
      title: " حساباتنا ",
      links: [
        { title: " واتساب ", url: "https://wa.me/+966590029919" },
        { title: " انستغرام ", url: "https://www.instagram.com/sada.alnoor7" },
        { title: " تيك توك ", url: "https://www.tiktok.com/@sada.alnoor" },
      ],
    },
  ];
  